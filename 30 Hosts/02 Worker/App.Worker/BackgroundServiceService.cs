...
/// <summary>
    ...
    {
        private static readonly string[] ExactScanAssemblyNames =
        {
            "App.Common",
            "App.Configuration",
            "App.Resources",
        ...
        private static readonly string[] ScanAssemblyPrefixes =
        {
            "App.Core.",
            "App.Infrastructure.",
            "App.Integrations.",
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            // bootstrap the configuration from the referenced assemblies
            var assemblies = LoadScanAssemblies(AppContext.BaseDirectory).ToArray();
        ...
        private static List<Assembly> LoadScanAssemblies(string baseDirectory)
        {
            return Directory.GetFiles(baseDirectory, "*.dll")
                ...
                .Where(item =>
                    ExactScanAssemblyNames.Contains(item.Name) ||
                    ScanAssemblyPrefixes.Any(prefix => item.Name.StartsWith(prefix, StringComparison.Ordinal)))
                .Select(item => Assembly.LoadFrom(item.File))
